...
class AddRecipeUIState {
    ...
    fun checkRicetta(): String? {
        ...
        for (ingrediente in ingredientsList!!) {
            if (ingrediente.name.isNullOrBlank()) return "Inserire il nome a tutti gli ingredienti"
            if (ingrediente.unitOfMeasure == null) return "Inserire una unità di misura a tutti gli ingredienti"
            if (ingrediente.quantity.isNullOrBlank() && ingrediente.unitOfMeasure != UnitOfMeasure.TOTASTE) return "Inserire una quantità a tutti gli ingredienti"
        }
